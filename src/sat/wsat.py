...
def encode(r, k, s, R, symbreak=True):
    ROWS = 1 << k
    OWN = ROWS - 1
    E = Enc()
    ...
    # (3) every row other than the own row lands inside R
    rejected = [t for t in range(1 << r) if t not in R]
    for v in range(ROWS):
        if v == OWN:
            continue
        for t in rejected:
            E.add(*[(-y[j][v] if (t >> j) & 1 else y[j][v]) for j in range(r)])
...
def verify(pat, r, k, R):
    ...
    if cover != ROWS - 1:
        return False, "coordinates uncovered"
    for v in range(ROWS):
        t = 0
...
if __name__ == "__main__":
    ...
    t0 = time.time()
    E, y, d = encode(args.r, args.k, args.s, R, symbreak=not args.no_symbreak)
    cnf = CNF(from_clauses=E.cls)
    tag = f"r={args.r} k={args.k} s={args.s} exp={args.k}/{args.s}"
